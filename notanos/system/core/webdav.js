...
function getJXONTree (oXMLParent) {
  ...
  if (oXMLParent.hasChildNodes()) {
    for (var oNode, sProp, vContent, nItem = 0; nItem < oXMLParent.childNodes.length; nItem++) {
      ...
      if (oNode.nodeType === 4) { sCollectedTxt += oNode.nodeValue; } /* nodeType is "CDATASection" (4) */
        ...
        if (oNode.prefix) {
					sProp = sProp.substr(oNode.prefix.length+1);
				}
  ...
  /* if (nLength > 0) { Object.freeze(vResult); } */
	...
	function xmlToJson(xml) {
		 return getJXONTree(xml);
	}
	
	var WebDav = {}
//hope you have a collapsing editor
      WebDav.mimeExtensions = {
	...
	WebDav.setData = function (name,data) {
			var request = new XMLHttpRequest();
			request.open('PUT', name, false);
			request.send(data);
		...
		function identifyContainer(container) {
			function fileNameIs(name) {
		...
		function sanitizeResponse(response) {
			...
			if (saneResponse.contentType === undefined) {saneResponse.contentType="unknown";}
			
			//need a more extensible way to do smart type identification
			if (saneResponse.container==true) {
				 identifyContainer(saneResponse);
			}
	...
	WebDav.getInfo = function(name) {
			...
			request.send();
			if (Math.floor(request.status/100)  == 2) {
				var jsonResponse =  xmlToJson(request.responseXML);
	...
	function getRawDirectory(name) {
		  //console.log("getRawDirectory('"+name+"');");
			...
			if (Math.floor(request.status/100)  == 2) {
				var jsonResponse =  xmlToJson(request.responseXML);
				return jsonResponse.multistatus.response;
			}
			return [];
	}	
	
	WebDav.getDirectoryListing = function (name) {
		  var list=getRawDirectory(name);
			var saneResponse = [];
			for (var i=1; i<list.length;i++) {
					saneResponse.push(sanitizeResponse(list[i]));
			}
			return saneResponse;
	}
	
	WebDav.loadFile_async = function(name,callback) {
			var request = new XMLHttpRequest();
			request.open('GET', name, true);
			request.onload=function (e) {callback(request.response)};
			request.send();		
	}

	WebDav.saveFile_async = function (name,data,callback) {
			var request = new XMLHttpRequest();
			request.open('PUT', name, true);
			request.send(data);
